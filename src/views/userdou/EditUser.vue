    ...
    <el-card class="form-card">
      ...
      >
        ...
        <div class="form-actions">
          <el-button @click="$router.push('/userDouList')">取消</el-button>
...
export default {
  ...
  methods: {
    loadUserData() {
      ...
      this.loading = true;
      request.get("/userdou/" + id).then(res => {
        if (res.code === '200') {
          this.form = res.data;
        } else {
    ...
    save() {
      this.$refs["ruleForm"].validate((valid) => {
        if (valid) {
          this.loading = true;
          request.put('/userdou/update', this.form).then(res => {
            if (res.code === '200') {
            ...
            this.$notify.error('更新失败：' + err.message);
          }).finally(() => {
            this.loading = false;
          });
        }
