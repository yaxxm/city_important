...
<!--          scope.row 就是当前行数据-->
          <div class="button-group">
            <el-button type="primary"style="margin-bottom: 10px;" @click="$router.push('/editAdmin?id=' + scope.row.id)">
              编辑
            </el-button>
            <el-button type="primary" @click="$router.push('/informAdmin?id=' + scope.row.id)">
              详情
            </el-button>
    ...
    return{
      ...
      rules:{
        newPass: [
          ...
          {min: 3, max: 10, message: '长度在3-10戈字符', trigger: 'blur'}
        ]
      }
    }
  },
